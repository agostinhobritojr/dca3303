...
void handleClient(boost::asio::ip::tcp::socket&& socket) {
  try {
    ...
    std::cout << "Cliente conectado: " << std::this_thread::get_id()
              << std::endl;
    std::cout << "Endereco remoto: " << socket.remote_endpoint().address()
              << std::endl;
    while (true) {
      ...
      // imprime a mensagem recebida e o id da thread que esta tratando o
      // cliente
      std::cout << "Mensagem recebida: " << std::this_thread::get_id() << ":"
                << message << std::endl;
...
int main() {
  try {
    ...
    // boost::asio::ip::tcp::endpoint
    // endpoint(boost::asio::ip::address::from_string("127.0.0.1"), 1234);
    ...
    while (true) {
      ...
      // note que a conexao eh transferida para a thread com std::move, que
      // invoca o construtor de movimentacao
      std::thread clientThread(handleClient, std::move(socket));

      // detach() eh usado para dexanexar a thread e esta nao seja destruida ao
      // sair do escopo, visto que clientThread eh uma variavel local
      clientThread.detach();
    }
  } catch (std::exception& e) {
    std::cerr << "Erro: " << e.what() << std::endl;
