...
long int calcularSomaComThreads(const std::vector<long int>& vetor,
  ...
  for (long int i = 0; i < numThreads; i++) {
    ...
    // funcao lambda para calcular a soma parcial de cada thread. o [&] indica
    // que as variaveis externas serao capturadas por referencia, ou seja, as
    // threads terao acesso as variaveis do escopo onde a funcao lambda foi
    // definida. isso permite que as threads acessem o vetor e as variaveis de
    // controle (inicio, fim e i) para calcular a soma parcial corretamente.

    // & variaveis capturadas por referencia. inicio, fim e i sao capturadas por
    // valor para evitar problemas de concorrencia. se fossem capturadas por
    // referencia, todas as threads poderiam acessar a mesma variavel, causando
...
int main() {
  ...
  std::cout << "Numero de cores: " << numCores << std::endl;
  if (numCores == 0) {
    numThreads = 1;
  } else {
    numThreads = numCores;
  ...
  std::cout << "Speedup = " << tempoSemThreads.count() / tempoComThreads.count()
            << "\n";
  return 0;
}